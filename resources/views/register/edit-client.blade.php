...
@section('content')
  <div class="max-w-2xl mx-auto">
    ...
    <form action="{{ route('clients.update', $client) }}" method="POST" class="space-y-6 bg-white p-8 rounded-lg shadow-lg">
      @csrf
      @method('PUT')

      <h2 class="text-lg font-semibold text-gray-900">Informações do cliente</h2>
      <p class="text-sm text-gray-500">Atualize os dados do cliente abaixo.</p>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <select id="type" name="type" required
            ...
            <option value="fisica" {{ old('type', $client->type) == 'fisica' ? 'selected' : '' }}>Pessoa Física</option>
            <option value="juridica" {{ old('type', $client->type) == 'juridica' ? 'selected' : '' }}>Pessoa Jurídica</option>
          </select>
        ...
        <div>
          ...
          <input id="document" name="document" value="{{ old('document', $client->document) }}"
                 class="mt-1 block w-full rounded-md border border-gray-300 bg-white py-3 px-4 text-sm shadow-sm placeholder-gray-400 focus:border-indigo-500 focus:ring-indigo-500" />
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="sm:col-span-2">
          ...
          <input id="street" name="street" value="{{ old('street', $client->street) }}" placeholder="Rua"
        ...
        <div>
          ...
          <input id="cep" name="cep" value="{{ old('cep', $client->cep) }}" placeholder="Rua"
                class="mt-1 block w-full rounded-md border border-gray-300 bg-white py-3 px-4 text-sm shadow-sm placeholder-gray-400
                focus:border-indigo-500 focus:ring-indigo-500" />
          @error('cep')
            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div>
          ...
          <input id="number" name="number" value="{{ old('number', $client->number) }}" placeholder="Número"
                 class="mt-1 block w-full rounded-md border border-gray-300 bg-white py-3 px-4 text-sm shadow-sm placeholder-gray-400 focus:border-indigo-500 focus:ring-indigo-500" />
        ...
        <div>
          ...
          <input id="city" name="city" value="{{ old('city', $client->city) }}" placeholder="Cidade"
                 class="mt-1 block w-full rounded-md border border-gray-300 bg-white py-3 px-4 text-sm shadow-sm placeholder-gray-400 focus:border-indigo-500 focus:ring-indigo-500" />
        ...
        <div>
          ...
          <input id="state" name="state" value="{{ old('state', $client->state) }}" placeholder="UF"
                 class="mt-1 block w-full rounded-md border border-gray-300 bg-white py-3 px-4 text-sm shadow-sm placeholder-gray-400 focus:border-indigo-500 focus:ring-indigo-500" />
      ...
      <div>
        ...
        <textarea id="notes" name="notes" rows="4"
                  class="mt-1 block w-full rounded-md border border-gray-300 bg-white py-3 px-4 text-sm shadow-sm placeholder-gray-400 focus:border-indigo-500 focus:ring-indigo-500">{{ old('notes', $client->notes) }}</textarea>
      ...
      <div class="flex items-center justify-between pt-4 border-t border-gray-100">
        ...
        <div class="flex items-center space-x-3">
          <button type="submit" class="inline-flex items-center px-6 py-2 bg-green-600 text-white text-sm font-semibold rounded-md shadow hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
            Salvar Alterações
          </button>
        </div>
      </div>
