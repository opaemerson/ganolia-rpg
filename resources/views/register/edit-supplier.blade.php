...
@section('content')
  <div class="max-w-xl mx-auto">
    ...
    <form action="{{ route('suppliers.update', $supplier->id) }}" method="POST" class="space-y-6 bg-white p-8 rounded-lg shadow-lg">
      @csrf
      @method('PUT')

      <h2 class="text-lg font-semibold text-gray-900">Editar Fornecedor</h2>
      <p class="text-sm text-gray-500">Atualize os dados do fornecedor abaixo.</p>
      ...
      <div>
        ...
        <input id="document" name="document" type="text" value="{{ old('document', $supplier->document) }}"
               class="mt-1 block w-full rounded-md border border-gray-300 bg-white py-3 px-4 text-sm shadow-sm placeholder-gray-400 focus:border-indigo-500 focus:ring-indigo-500" />
