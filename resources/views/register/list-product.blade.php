...
@section('top_actions')
  <a href="{{ route('products.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-md shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500">
    Criar
  </a>
@endsection
  ...
  <div class="mb-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <form method="GET" action="{{ route('products.index') }}" class="flex items-center space-x-2">
        ...
        <button type="button" onclick="window.location='{{ route('products.index') }}'" class="inline-flex items-center px-3 py-2 bg-gray-100 text-gray-700 rounded-md hover:bg-gray-200 text-sm">
          Limpar
        </button>
      </form>
    </div>
    ...
    <div class="overflow-x-auto">
      @if($products->count())
        <table class="min-w-full divide-y divide-gray-200">
          ...
          @php
            $start = method_exists($products, 'firstItem') && $products->firstItem() ? $products->firstItem() : 1;
          @endphp
            ...
            @foreach($products as $index => $product)
              <tr class="hover:bg-gray-50">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                  <a href="{{ route('products.edit', $product->id) }}" class="text-blue-400 hover:text-blue-200">Editar</a>
      ...
      <div class="px-4 py-3 bg-white border-t border-gray-100 sm:px-6">
        <div class="flex items-center justify-between">
          <div class="text-sm text-gray-700">
            Mostrando <span class="font-medium">{{ $products->firstItem() ?? 0 }}</span> até <span class="font-medium">{{ $products->lastItem() ?? 0 }}</span> de <span class="font-medium">{{ $products->total() ?? $products->count() }}</span>
          </div>
          <div>
