...
<div
  ...
  x-data="saleForm({ products: {{ Js::from($products) }}, clients: {{ Js::from($clients) }} })"
  x-cloak
>
  <form action="{{ route('sales.store') }}" method="POST" @submit="prepareSubmit">
    @csrf
    ...
    <template x-if="products.length === 0">
      <div class="text-sm text-red-600 mb-4">
        Nenhum produto cadastrado. Cadastre um produto antes de criar uma venda.
      </div>
    </template>
    ...
    <template x-for="(item, index) in items" :key="item.uid">
      <div class="flex items-center gap-3 mb-3 p-3 border border-gray-200 rounded-md bg-gray-50">
        ...
        >
          {{-- server-side fallback so names always appear --}}
...
<script>
function saleForm({ products = [], clients = [] }) {
  return {
    products,
    clients,
    ...
    onProductChange(index) {
      ...
      if (prod) {
        ...
        // fallback: read data-price attribute from option
