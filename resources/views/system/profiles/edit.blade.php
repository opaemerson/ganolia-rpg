...
@section('content')
    <div class="max-w-xl mx-auto">
        <form action="{{ route('profiles.update', $profile->id) }}" method="POST"
            ...
            <p class="text-sm text-gray-500">Atualize os dados do perfil abaixo.</p>

            <div>
                <label for="name" class="block text-sm font-medium text-gray-700">Nome</label>
                <input id="name" name="name" type="text" value="{{ old('name', $profile->name) }}" required autofocus
            ...
            <div class="rounded-lg border border-amber-200 bg-amber-50 p-4">
                <p class="text-sm text-amber-900">
                    Este perfil possui <strong>{{ $profile->users_count ?? 0 }}</strong> usuários vinculados.
                </p>
            </div>
            ...
            <div class="flex items-center justify-between pt-4 border-t border-gray-100">
                ...
                <button type="submit"
                    class="inline-flex items-center px-5 py-2 bg-green-600 text-white text-sm font-semibold rounded-md shadow hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                    Salvar Alterações
                </button>
            </div>
        </form>
